...
build: ## Build app
	go build ${LDFLAGS} -o bin/streampay main.go

run: ## Run application (without building)
	./bin/streampay serve-web --config=$$(pwd)/config/streampay.yml

run-kms: ## Run KMS (without building)
	./bin/streampay serve-kms --config=$$(pwd)/config/streampay.yml

run-scheduler: ## Run Scheduler (without building)
	./bin/streampay run-scheduler --config=$$(pwd)/config/streampay.yml
...
docker-build: ## Build docker image for Oxygen
	DOCKER_BUILDKIT=1 docker build -t streampay-local .

docker-local:
	docker-compose -f docker-compose.local.yml up

clean-test-dbs: ## Drop test "streampay_test*" databases
	psql -c "\l" | grep "streampay_test" | awk '{print $$1}' | xargs -I {} psql -c "drop database {};"