...
const App: React.FC = () => {
    ...
    React.useEffect(() => {
        if (payment) {
            document.title = `StreamPayments: ${payment.merchantName}`;
        }
    }, [payment]);

    React.useEffect(() => {
        if (paymentLink) {
            document.title = `StreamPayments Link: ${paymentLink.merchantName}`;
        }
    }, [paymentLink]);
    ...
    return (
        <PaymentContext.Provider value={{payment, setPayment}}>
            <PaymentLinkContext.Provider value={{paymentLink, setPaymentLink}}>
                <main className="min-h-screen bg-main-blue-3">
                        ...
                        {toggled("show_branding") && (
                            <div className="pt-2 pb-4 text-sm sm:hidden">
                                <p className="text-center text-gray-500">
                                    ...
                                    <a className="color-streampay" target="_blank" href="https://streampayments.co">
                                        Stream Protocol. All Rights Reserved.
                                    </a>
                                </p>
                            </div>
                        )}
