...
const defaultMenus: MenuItem[] = [
    ...
    {
        path: "https://docs.streampayments.co/",
        name: "Documentation"
    }
];
...
const App: React.FC = () => {
    ...
    React.useEffect(() => {
        ...
        if (location.pathname === "/login") {
            document.title = "Login | Dashboard | StreamPay";
            return;
        }

        const pageName = menus.find((item) => item.path === location.pathname)?.name;
        document.title = `${pageName} | Dashboard | StreamPay`;
    }, [location, merchants]);
    ...
    const sendSupportForm = async (value: SupportMessage, form: FormInstance<SupportMessage>) => {
        try {
            ...
            notificationApi.info({
                ...
                icon: <CheckOutlined style={{color: "#4377bf"}} />
            });
                                    ...
                                    {(routeCtx: RouteContextType) => (
                                        <div className={b("logo")}>
                                            ...
                                            {routeCtx.isMobile ? null : (
                                                <span className={b("logo-text")}>StreamPay</span>
                                            )}
                                        </div>
                                    )}
                                </RouteContext.Consumer>
